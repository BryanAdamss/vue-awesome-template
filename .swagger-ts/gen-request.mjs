/**
 * @author GuangHui
 * @description 基于openapi-typescrip生成文件生成openapi-typescript-fetch请求方法
 */
...
import { SWAGGER_API_FILE, SWAGGER_REQUEST_OUTPUT_FILE } from './const.mjs'
...
function genRequestFile() {
  const swgApiTsContent = fs.readFileSync(
    path.join(process.cwd(), SWAGGER_API_FILE),
    'utf-8',
  )

  const interfacePathCont = getInterfacePathsCont(swgApiTsContent)
  ...
  strArr.unshift(`import { fetcher } from \'@/services/instance/otfetch-instance\'${EOL}`)
  ...
  */${EOL}`)
  ...
  const outputFilePath = path.join(process.cwd(), SWAGGER_REQUEST_OUTPUT_FILE)
